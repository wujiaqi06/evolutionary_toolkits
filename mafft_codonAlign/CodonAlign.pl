...
my %opts;
getopts('c:p:o:t:', \%opts);
my $input_cds = $opts{'c'} or die "use: perl $0 -c input_cds_folder -p input_pep_align_folder -o output_fodler -t codon_table\n";
my $input_pep_align = $opts{'p'} or die "use: perl $0 -c input_cds_folder -p input_pep_align_folder -o output_fodler -t codon_table\n";
my $output_folder = $opts{'o'} or die "use: perl $0 -c input_cds_folder -p input_pep_align_folder -o output_fodler -t codon_table\n";
my $codon_table = $opts{'t'} or die "use: perl $0 -c input_cds_folder -p input_pep_align_folder -o output_fodler -t codon_table\n";
...
foreach my $i (@folder1){
	...
	foreach my $j (@seq_ori_name){
		...
		if (($j eq "NC 001892.1")&($i =~ /ATP6/)){
			print "$j, trans: @translate_seq\n";
			print "$j, align: @align_seq\n";
		}
		#print "$j, trans: @translate_seq\n";
		#print "$j, align: @align_seq\n";
